"""
Comprehensive Benchmark Harness for Drift-Sense++ HCR Pipeline
Evaluates on held-out test sets with zero calibration leakage.
Reports: accuracy, recall, latency, CDF, difficulty breakdown, Pareto frontier.
"""
...
from inference_hcr import perform_hcr_localization
...
def evaluate_on_manifest(manifest_path, label=""):
    ...
    for method_name in methods:
        ...
        for idx, row in df.iterrows():
            ...
            elif method_name == "HCR_Pipeline":
                x_pred, y_pred, meta = perform_hcr_localization(ref_img, search_img)
    ...
    # Error histogram for HCR
    if "HCR_Pipeline" in methods and isinstance(methods["HCR_Pipeline"], dict):
        errs = methods["HCR_Pipeline"]["errors"]
...
def main():
    ...
    # Evaluate on original 120-case benchmark
    results_120 = evaluate_on_manifest(ORIGINAL_MANIFEST, "Original 120-Case Benchmark")

    # Evaluate on held-out test set (if available)
    results_holdout = evaluate_on_manifest(HOLDOUT_MANIFEST, "Held-Out Test Set (Unseen Seeds)")

    # Save results
    if results_120:
        rows = []
        for method, stats in results_120.items():
            if isinstance(stats, dict):
                rows.append({
                    "Method": method,
                    "Dataset": "Original_120",
        ...
        if results_holdout:
            for method, stats in results_holdout.items():
                if isinstance(stats, dict):
                    rows.append({
                        "Method": method,
        ...
        csv_path = "results/hcr_benchmark_results.csv"
