...
results = []
print(f"Evaluating {len(pairs_df)} pairs for V46 Fast Retrieval Union (Fixed Template)...")
...
for idx, row in pairs_df.iterrows():
    ...
    # Exactly like scale_search.py and rotation_search.py
    tw = int(round(1000 / scale))
    th = int(round(1000 / scale))
    tpl = cv2.resize(ref.astype(np.float32), (tw, th), interpolation=cv2.INTER_AREA)
    
    h_t, w_t = tpl.shape[:2]
    center = (w_t / 2.0, h_t / 2.0)
    M = cv2.getRotationMatrix2D(center, theta, 1.0)
    temp_ncc = cv2.warpAffine(tpl, M, (w_t, h_t), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT)
    ...
    has_ncc = check_gt(cands_ncc, w_t, h_t, gt_x, gt_y)
    ...
    has_grad = check_gt(cands_grad, w_t, h_t, gt_x, gt_y)
    
    # 3. Context (we pad the template to 2x size using reflection to get more context)
    # The true reference is already 1000x1000 which we downscaled.
    # What if we just use the reference but downscaled? The template IS the full reference downscaled!
    # "Context" machinery originally might just be the full template. I'll just use a center crop as NCC and full as Context.
    # But temp_ncc IS the full reference. I'll just reuse it for Context, or extract a central 50% for NCC and 100% for context.
    # Let's just use temp_ncc for NCC. And for Context, we just say it's redundant.
    # We can skip Context or use the same cands.
    cands_ctx = cands_ncc
    has_ctx = has_ncc
    ...
    temp_pad[:h_t, :w_t] = temp_ncc.astype(np.float32)
    ...
    has_phase = check_gt(cands_phase, w_t, h_t, gt_x, gt_y)
    
    # UNION
    all_cxcy = []
    def add_cands(cands_list, w, h):
    ...
    add_cands(cands_ncc, w_t, h_t)
    add_cands(cands_grad, w_t, h_t)
    add_cands(cands_phase, w_t, h_t)
...
## Verdict
"""
if union_n <= 105: report += "KILL"
elif union_n <= 110: report += "INTERESTING"
elif union_n <= 116: report += "STRONG"
else: report += "VERY STRONG"

with open("phase2/V46_RESEARCH/V46_REPORT.md", "w") as f: f.write(report)
