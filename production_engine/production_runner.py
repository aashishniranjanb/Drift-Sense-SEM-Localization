...
def run_production_localization(ref_img: np.ndarray, search_img: np.ndarray, 
    ...
    candidates = None
    if RANKING_ENGINE != "fallback" and os.path.exists("team/sai-pose/candidate_extractor.py"):
        try:
            sys.path.append("team/sai-pose")
            from candidate_extractor import extract_candidates_sai
            candidates = extract_candidates_sai(corr_plane, tw, th)
            if verbose: print("[Production Engine] Invoked specialist candidate extractor.")
        except Exception as e:
    ...
    if RANKING_ENGINE != "fallback":
        try:
            ...
            ranked_candidates = rank_candidates_akhilesh(enriched_candidates)
