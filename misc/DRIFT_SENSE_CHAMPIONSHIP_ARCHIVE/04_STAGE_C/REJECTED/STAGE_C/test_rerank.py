...
def main():
    ...
    for pid in ranking_failures:
        ...
        # Build the pairwise set: V54 + top 3 alternatives
        pairwise_set = []
        if np.hypot(v54_cand_match["cx"] - v54_x, v54_cand_match["cy"] - v54_y) < max(25, tw*0.25):
            pairwise_set.append(v54_cand_match)
            
        for c in scored_cands:
            if c not in pairwise_set:
                pairwise_set.append(c)
            if len(pairwise_set) >= 4:
                break
                
        # Re-rank pairwise set by ctx_s128
        pairwise_set.sort(key=lambda x: x["ctx_s128"], reverse=True)
        
        top_pairwise = pairwise_set[0]
        dist_to_gt = np.hypot(top_pairwise["cx"] - gt_x, top_pairwise["cy"] - gt_y)
        if dist_to_gt < max(25, tw*0.25):
            rescues += 1
            print(f"RESCUED {pid}! GT won pairwise context_s128.")
            
    print(f"Total rescues using ctx_s128 on pairwise set: {rescues} / {len(ranking_failures)}")
