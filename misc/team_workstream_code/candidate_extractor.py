...
def extract_candidates_akhilesh(corr_plane: np.ndarray, tw: int, th: int,
                                      ref_img: np.ndarray = None, search_img: np.ndarray = None,
                                      est_scale: float = 10.0, est_theta: float = 0.0,
                                      max_final_k: int = 200) -> list:
    """
    ...
    pool_raw = extract_nms_fast(corr_plane, tw, th, max_k=max_final_k * 3, r=5)
    if len(pool_raw) <= max_final_k:
        return pool_raw
    ...
    # Take 70% from center and 30% from periphery
